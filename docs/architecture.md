...
- A metadata row exists only after a committed upload; an interrupted upload (or a best-effort object delete that fails) leaves an orphan object with no row. An orphan is harmless — with no metadata it is invisible to every API — so it is never deleted eagerly (no commit-time rewrite, no full-bucket scan). Nothing records an in-flight upload either: `WriteFile` only signs a URL, keeping the upload path a single round-trip rather than paying a write to mark intent. Reclamation is therefore left to operators, and it cannot be a naive age-based bucket lifecycle rule, since orphans share the live files' key namespace; it must be a list-and-reconcile pass (O(objects)) run out of band if storage cost ever warrants it. Deletes remove the metadata row first (the source of truth) and best-effort the object second.
