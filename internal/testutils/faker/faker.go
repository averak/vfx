// Package faker produces deterministic test data.
// Functions here MUST return the same value for the same input across runs and machines; this is what lets tests assert on concrete IDs without flake.
package faker
...
//
// Use this in fixtures and assertions when you need an identifier the test can reference by a human name (faker.UUIDv5("player-1")) rather than a freshly-generated UUID that has to be plumbed back through assertions.
